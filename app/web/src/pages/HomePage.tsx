      ...
      <div className="grid grid-cols-[1.1fr_0.9fr] gap-6">
        <div className="flex flex-col gap-6">
          <Section icon="📢" title="최신 공지사항">
            <div className="flex flex-col gap-3">
              {noticeLoading ? (
                <div className="card p-4 text-slate-500">공지 불러오는 중...</div>
              ) : noticeError ? (
                <div className="card p-4 text-rose-600">공지 불러오기 실패: {noticeError}</div>
              ) : notices.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          <Section icon="🎬" title="핫클립 (하이라이트)">
            {hotClips.length === 0 ? (
              <div className="card p-10 text-slate-400 text-center">
                핫클립 썸네일을 추가해주세요.
              </div>
            ) : (
              <div className="flex flex-col gap-4">
